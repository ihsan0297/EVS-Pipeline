...
qr_ocr_module.py
────────────────────────────────────────────────────────────────────────────
Call `process_images(list_of_image_paths, model_path="best.pt")`
and get a Python dict ready for `json.dumps`, shaped exactly like

    { "qr_Res": [ {image_path: …, detection: …, fast_qr: …, ocr: …}, … ] }

All helper routines are private (prefixed with “_”).  Nothing is written to
disk unless you change the two *_DIR constants or turn them off.
───────────────────────────────────────────────────────────────────────────
"""
...
def process_images(
    ...
    ----------
    image_paths : list[str]
        Absolute or relative file paths (JPEG/PNG/TIFF …) in temporal order.
    model_path  : str
    ...
    -------
    dict
        { "qr_Res": [ …one result per input image… ] }
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        # stitching logic —
        #   detect edge boxes quickly to decide if we should combine frames
        img = cv2.imread(img_path)
        if img is None:
            continue
        H = img.shape[0]
        ...
        # try to pair a previous bottom with a current top
        if prev_bottom and tops:
            xb1, _, xb2, _ = prev_bottom["box"]
            paired = False
            for t in tops:
                xt1, _, xt2, _ = t["box"]
                ov = min(xb2, xt2) - max(xb1, xt1)
                if ov > 0.5*min(xb2-xb1, xt2-xt1):
                    # stitch and analyse
                    ...
                    if it is not None and ib is not None:
                        ...
                        stitched_file = _STITCHED_DIR / f"stitch_{prev_bottom['idx']}_{idx}.png"
                        cv2.imwrite(stitched_file.as_posix(), stitched)
                        results.append(_analyse_one(stitched_file.as_posix(),
                                                    model, wechat))
                    prev_bottom = None
                    paired = True
                    break
            if not paired:
                prev_bottom = max(
                    bots, key=lambda d:(d["box"][2]-d["box"][0])*
...
#     # Example: process every .jpg/.png in a folder called “stitch”
#     exts   = (".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff")
#     images = sorted(sum((glob.glob(f"stitch/*{e}") for e in exts), []))

#     data = process_images(images)
#     with open("results.json", "w", encoding="utf-8") as f:
#         json.dump(data, f, indent=2, ensure_ascii=False)

#     print(f"[OK] wrote {len(data['qr_Res'])} records to results.json")
