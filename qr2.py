...
# builtins.print = lambda *args, **kwargs: None  # Replace print with no-op function
...
def _analyse_one(img_path: str, model: YOLO, wechat) -> Dict[str, Any]:
    ...
    H, W  = img.shape[:2]
    boxes = (model(img_path,verbose=False)[0].boxes.xyxy.cpu().numpy()
             .astype(float).tolist())
...
if __name__ == "__main__":
    # Example usage
    image_paths = glob.glob("test_images/*.jpg")  # Adjust the path as needed
